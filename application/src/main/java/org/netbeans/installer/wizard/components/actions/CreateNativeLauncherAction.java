...
public class CreateNativeLauncherAction extends WizardAction {
    ...
    public static final String WINDOWS_COMPATIBLE_JVM_PROPERTY =
        "nbi.windows.compatible.jvm";
    ...
    public void execute() {
        ...
        try {
            ...
            LogManager.log("CreateNativeLauncher.execute:" +  WINDOWS_COMPATIBLE_JVM_PROPERTY + "\"" + System.getProperty(WINDOWS_COMPATIBLE_JVM_PROPERTY) + "\"");
            if (System.getProperty(WINDOWS_COMPATIBLE_JVM_PROPERTY) != null) {
                String compatibleJVMs[] = System.getProperty(WINDOWS_COMPATIBLE_JVM_PROPERTY).split(";");
                for (String compatibleJVM : compatibleJVMs) {
                    String vmFields[] = compatibleJVM.split(",");
                    if (vmFields.length > 0) {
                        Map<String, String> fieldMap = new HashMap<>();
                        for (String field : vmFields) {
                            String argValue[] = field.split("=");
                            if (argValue.length == 2) {
                                fieldMap.put(argValue[0], argValue[1]);
                            }
                        }

                        if(fieldMap.keySet().containsAll(Set.of("minVersion"))) {
                            properties.addCompatibleJava(new JavaCompatibleProperties(
                                fieldMap.get("minVersion"),
                                fieldMap.get("maxVersion"),
                                fieldMap.get("vendor"),
                                fieldMap.get("osName"),
                                fieldMap.get("osArch")));
                        }
